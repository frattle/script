...
branch=oreo-m3-release
main_branch=oreo-m2-release
aosp_version=OPM3.171019.019
aosp_version_real=OPM3.171019.019
aosp_tag=android-8.1.0_r21
...
for repo in "${aosp_forks[@]}"; do
  ...
  if [[ -n $build_number ]]; then
    ...
    git cherry-pick android-8.1.0_r20..oreo-m2-release
...
for kernel in ${!kernels[@]}; do
  ...
  if [[ -n $build_number ]]; then
    ...
    git checkout oreo-m2-release || exit 1
    git checkout -B $branch || exit 1
    git push -f -u origin $branch || exit 1
  fi
